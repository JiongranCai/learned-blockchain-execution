...
if [[ "$#" -lt 2 || "$#" -gt 3 ]]; then
  echo "usage: $0 RUN_ID preflight|pilot|formal [--resume]" >&2
  exit 2
fi

readonly run_id="$1"
readonly stage="$2"
if [[ "${stage}" != "preflight" && "${stage}" != "pilot" && "${stage}" != "formal" ]]; then
  echo "stage must be preflight, pilot, or formal" >&2
  exit 2
fi
readonly resume="${3:-}"
...
mapfile -t configs < <(find "${run_dir}/configs/${stage}" -type f -name '*.json' -print | sort)
if [[ "${#configs[@]}" -eq 0 ]]; then
  echo "no ${stage} configs under ${run_dir}" >&2
  exit 1
fi
...
for config in "${configs[@]}"; do
  ...
  if [[ "${resume}" == "--resume" && -f "${run_records}" ]]; then
    ...
    if [[ "${actual_records}" -ne "${expected_records}" ]]; then
      echo "refusing to append to partial matrix: ${run_records} (${actual_records}/${expected_records})" >&2
      exit 1
    fi
    if grep -Eq '"status":"(failed|censored_timeout)"|"censored":true|"canonical_match":false' "${run_records}"; then
